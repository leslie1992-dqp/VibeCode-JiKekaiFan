...
基于 **Spring Boot 3** 与 **Vue 3** 的前后端分离外卖点餐演示项目，涵盖账号体系（登录 / 注册 / 重置密码）、商家检索与排序、商家详情与秒杀券、购物车与草稿下单、订单全生命周期、优惠券、评价与图片上传、个人主页等。仓库内提供与代码同步的 **Wiki**（ER 图、API 映射、业务流程、秒杀与 Kafka/Redis 说明）。
...
- **订单列表**：展示全部订单及状态（待支付、配送中、已完成、已取消等）。

![main_page](/images/2.png)

- 支持 **取消订单**；可区分 **成功交易**、**待支付** 等展示。

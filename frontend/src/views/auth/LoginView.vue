<template>
  ...
  <section class="card auth-card">
    ...
    <p class="auth-lead">使用手机号与密码进入即刻开饭</p>

    <form class="auth-form" :aria-busy="submitting" @submit.prevent="handleLogin">

      <div class="form-row auth-field">

        <label class="auth-label" for="login-mobile">手机号</label>
...
async function handleLogin() {
  ...
  try {
    ...
    await store.login({ mobile: mobile.value.trim(), password: password.value });
    ...
    showToast("error", getApiErrorMessage(err, "登录失败"));
